...
type IamServiceAccountStaticAccessKey struct {
	...
	// ID of the Lockbox secret version that contains the values of `accessKey` and `secretKey`. This is only populated when `outputToLockbox` is supplied. This version will be destroyed when the IAM key is destroyed, or when `outputToLockbox` is removed.
